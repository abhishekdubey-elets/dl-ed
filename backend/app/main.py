...
async def lifespan(_: FastAPI) -> AsyncGenerator[None, None]:
    # Refuse to serve production traffic on the dev signing key: every token
    # ever issued would be forgeable by anyone who has read the source.
    if settings.is_production and settings.SECRET_KEY.startswith("CHANGE_ME"):
        raise RuntimeError(
            "SECRET_KEY is still the insecure development default. Set a real "
            "one (openssl rand -hex 32) before running with ENVIRONMENT=production."
        )
    logger.info(
