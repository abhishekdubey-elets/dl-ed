...
# A managed database (Supabase, RDS, ...) arrives as one DATABASE_URL rather
# than POSTGRES_* parts — wait on ITS host/port, not the compose defaults.
...
parts = urlsplit(url)
print(f"HOST='{parts.hostname or 'postgres'}'")
